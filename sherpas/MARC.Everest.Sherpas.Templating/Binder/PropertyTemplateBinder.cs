...
namespace MARC.Everest.Sherpas.Templating.Binder
{
    ...
    public class PropertyTemplateBinder : IArtifactBinder
    {
        ...
        /// </summary>
        private static void UpdateMethodToPath(MethodDefinitionBase method, Type containerType, string containerName)
        {
            ...
            foreach (var ins in method.Instruction)
            {
                ...
                else if (ins is ConditionalStatementDefinition)
                {
                    ...
                    if (asd.PropertyName != null && realType != containerType && asd.Operator != OperatorType.NotContains)
                    {
                        asd.Instruction.Insert(0, new ConditionalStatementDefinition()
                        {
                            PropertyName = ResolvePropertyInfo(asd.PropertyName, realType).Name,
        ...
        public void Bind(BindingContext context)
        {
            ...
            foreach (var mi in propertyTemplate.Initialize)
                UpdateMethodToPath(mi, propertyTraversalType, propertyTemplate.Name);
            ...
            //if(originalTemplate.Templates.Count > 0) // TODO: Only do this if there are any changes
            //{
